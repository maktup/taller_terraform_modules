...
resource "aws_instance" "ec2_generic" {
  count                       = "${length(local.const_lista_datos_instancia)}"                                                                 //IMPORTANTE: Obtiene el TAMAÑO de la 'LISTA' para iterar.
  ami                         = "${var.const_tipo_imagen_redhat}"                                                                              //'IMAGEN' de REDHAT.
  instance_type               = "${var.const_tipo_instancia}"                                                                                  //Tipo de 'IMAGEN' pequeña.
  private_ip                  = "${local.const_lista_datos_instancia[count.index].ip}"                                                         //Obtiene las IPs 'DINÁMICAMENTE' de la LISTA. 
  subnet_id                   = "${aws_subnet.sn_generic[local.const_lista_datos_instancia[count.index].posicion].id}"                         //Obtener el ID de la 'SUBNET' (posiciones 'DINÁMICAMENTE' de la LISTA).   
  vpc_security_group_ids      = ["${aws_security_group.sg_generic.id}"]                                                                        //Obtener los IDs de 'SECURITY-GROUPs'.
  key_name                    = "${element("${[aws_key_pair.kp_001.id, aws_key_pair.kp_002.key_name, aws_key_pair.kp_003.id]}", count.index)}" //Obtener los IDs de 'KEY-PAIRs'.  user_data                   = "${file("${var.const_directorio_script_sh}")}"                                         //Referencia al archivo: '.sh'.
